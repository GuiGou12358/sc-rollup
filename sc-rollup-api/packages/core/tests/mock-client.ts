...
class StringEncoder implements RawTypeEncoder<KvRawType, ActionRawType> {
  ...
  encodeSetQueueHead(index: number): ActionRawType {
    const encodedNumberWithoutPrefix = simpleTypeCoder
      .encodeNumber(index, "u32")
      .slice(2)
...
export function resetRemoteStorage() {
  ...
  remoteValues.set(
    simpleTypeCoder.encodeNumber(1, "u32"),
    simpleTypeCoder.encodeString("message 1"),
  )
  remoteValues.set(
    simpleTypeCoder.encodeNumber(2, "u32"),
    simpleTypeCoder.encodeString("message 2"),
  )
  remoteValues.set(
    simpleTypeCoder.encodeNumber(3, "u32"),
    simpleTypeCoder.encodeString("message 3"),
  )
  // the version number
...
function checkConditions(conditions: KvRawType[]) {
...
function updateRemoteValues(updates: KvRawType[]) {
  updates.forEach((c) => {
    const key = c[0] as HexString
    const value = c[1]
    if (value) {
...
function doRemoteActions(actions: ActionRawType[]) {
  actions.forEach((v) => {
