...
test("Read / Write values", async () => {
  ...
  expect(remoteValues.get(VERSION_NUMBER_KEY)).toBe(
    simpleTypeCoder.encodeNumber(33, "u32"),
  )
  ...
  expect(remoteValues.get(KEY_2)).toBe(
    simpleTypeCoder.encodeString("Hello Dot"),
  )
  expect(remoteValues.get(KEY_3)).toBe(simpleTypeCoder.encodeBoolean(false))
  expect(remoteValues.get(KEY_4)).toBeUndefined
  expect(remoteValues.get(otherKey)).toBe(
    simpleTypeCoder.encodeBigInt(BigInt(52), "u128"),
  )
  // version number must be updated
  expect(remoteValues.get(VERSION_NUMBER_KEY)).toBe(
    simpleTypeCoder.encodeNumber(34, "u32"),
  )
})
