...
The number of attempts a player is allowed depends on the **NFTs hold by the player**. Each NFT defines a maximum number of attempts through a specific attribute.
...
The `squid indexer` indexes all events coming from the two ink! smart contracts.

The worker:
 - reads  the on-chain data from the `ink! smart contract`
 - uses a `VRF` to compute the target number
 - query the indexer to determine the maximum number of attempts available to the user (Graph Api Oracle). This number depends on the NFTs held by the user’s address. If no NFTs are owned by the address, the default number of attempts is 3.
