...
//! ### Token Creation
//!
//! Token creation start by calling the `mint(&mut self, id: u32, max_attempts)` function.
//! The token owner becomes the function caller. The Token ID and Max Attempts need to be specified
//! as the argument on this function call.
//! Max Attempts is used by 'the Guess The Number' game
//!
//! ### Token Transfer
//!
...
mod erc721 {
    ...
    pub struct Erc721 {
        ...
        /// Max Attempts is used by 'the Guess The Number' game
        max_attempts: Mapping<TokenId, MaxAttempts>,
    }
    ...
    /// Event emitted when a token is burnt
    #[ink(event)]
    pub struct Burnt {
        #[ink(topic)]
        id: TokenId,
    ...
    impl Erc721 {
        ...
        /// Returns the max attempts (used by 'the Guess The Number' game) of the token.
        #[ink(message)]
        pub fn get_max_attempts(&self, id: TokenId) -> Option<MaxAttempts> {
            self.max_attempts.get(id)
        }
        ...
        /// Creates a new token.
        #[ink(message)]
        pub fn mint(&mut self, id: TokenId, max_attempts: MaxAttempts) -> Result<(), Error> {
            let caller = self.env().caller();

            if self.max_attempts.contains(id) {
                return Err(Error::TokenExists);
            }
            self.max_attempts.insert(id, &max_attempts);
        ...
        pub fn burn(&mut self, id: TokenId) -> Result<(), Error> {
            ...
            max_attempts.remove(id);
            self.clear_approval(id);

            self.env().emit_event(Burnt {
                id,
            });
    ...
    mod tests {
        ...
        fn mint_works() {
            ...
            // Create token Id 1.
            assert_eq!(erc721.mint(1, 5), Ok(()));
            // Alice owns 1 token.
            assert_eq!(erc721.balance_of(accounts.alice), 1);
            // Max Attempts of the token Id 1
            assert_eq!(erc721.get_max_attempts(1), Some(5));
        }
        ...
        fn mint_existing_should_fail() {
            ...
            assert_eq!(erc721.mint(1, 5), Err(Error::TokenExists));
        }
        ...
        fn transfer_works() {
            ...
            assert_eq!(3, ink::env::test::recorded_events().len());
            // Bob owns token 1
        ...
        fn burn_works() {
            ...
            assert_eq!(erc721.owner_of(1), None);
            // Max Attempts of the token Id 1
            assert_eq!(erc721.get_max_attempts(1), None);
        }
        ...
        fn burn_clears_approval() {
            ...
            assert_eq!(erc721.mint(1, 7), Ok(()));
            // Max Attempts of the token Id 1
            assert_eq!(erc721.get_max_attempts(1), Some(7));
            // Set caller to Bob
            set_caller(accounts.bob);
        ...
        fn test_max_attempts() {
            ...
            // Max Attempts of the token Id 1
            assert_eq!(erc721.get_max_attempts(1), Some(5));
            // Token 2 does not exists.
            assert_eq!(erc721.get_max_attempts(2), None);
            // Destroy token Id 1.
            assert_eq!(erc721.burn(1), Ok(()));
            // Token 2 does not exist anymore.
            assert_eq!(erc721.get_max_attempts(1), None);
        }
